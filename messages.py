    ...
    if state == 0:
        ...
        if message == globals.BTN_ORDER[db_user['lang_id']]:
            ...
            if context.user_data.get("carts", {}):
                ...
                update.message.reply_text(
                    text=f"<b>{globals.INFO[db_user['lang_id']]}</b>\n\n"
                         f"👤 <b>{globals.NAME_SURNAME[db_user['lang_id']]}</b> {db_user['first_name']}\n"
                         f"📞 <b>{globals.PHONE[db_user['lang_id']]}</b> {db_user['phone_number']} \n\n"
                         f"📥 <b>{globals.ZAKAZLAR[db_user['lang_id']]}</b> \n"
                         f"{text}",
                    parse_mode='HTML'
                )
            ...
            update.message.reply_text(f"{globals.YOZING[db_user['lang_id']]}")
        ...
        update.message.reply_text(f"{globals.RAHMAT[db_user['lang_id']]}")
